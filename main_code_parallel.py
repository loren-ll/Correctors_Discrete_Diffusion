...
def _run_one_forward_particle(w, mu, beta, checkpoint_times, seed):
    """
    Worker: generate one forward particle at all checkpoint times.
    Called in parallel by generate_forward_samples_at_checkpoints.
    """
    rng = np.random.default_rng(seed)
    x0 = sample_from_pi(w, mu, rng=rng)
    result = {}
    for t in checkpoint_times:
        if t == 0.0:
            result[float(t)] = x0.copy()
        else:
            result[float(t)] = forward_marginal(x0, beta, t, rng=rng)
    return result


def generate_forward_samples_at_checkpoints(w, mu, beta, checkpoint_times, n_mc,
                                            MASK=-1, n_jobs=-1):
    """
    Generate forward samples at checkpoint times using marginal distributions.
    Particles are generated in parallel across available CPU cores.
    ...
    n_jobs : int
        ...
        Set to the number of CPUs allocated in your SLURM script.
    ...
    # Spawn independent seeds for each particle — guarantees reproducibility
    # and statistically independent RNG streams across workers.
    ss = np.random.SeedSequence()
    seeds = ss.spawn(n_mc)

    results = Parallel(n_jobs=n_jobs, backend='loky')(
        delayed(_run_one_forward_particle)(w, mu, beta, checkpoint_times, seeds[m])
        for m in range(n_mc)
    )

    # Reassemble into (n_mc, N) arrays keyed by time
    forward_samples = {float(t): np.empty((n_mc, N), dtype=np.int16)
                       for t in checkpoint_times}
    for m, particle_dict in enumerate(results):
        for t, x in particle_dict.items():
            forward_samples[t][m] = x
...
def _run_one_gillespie_particle(N, w, mu, beta, T, checkpoint_times, seed):
    """
    Worker: run one Gillespie reverse trajectory.
    Called in parallel by add_gillespie_reverse.
    """
    rng = np.random.default_rng(seed)
    ...
    return times, ckpt_samples


def add_gillespie_reverse(samples, w, mu, beta, T, n_jobs=-1):
    """
    ...
    # Spawn independent seeds — one per particle
    ss = np.random.SeedSequence()
    seeds = ss.spawn(n_mc)

    print("Running Gillespie reverse process (parallel)...")
    results = Parallel(n_jobs=n_jobs, backend='loky', verbose=1)(
        delayed(_run_one_gillespie_particle)(N, w, mu, beta, T, checkpoint_times, seeds[m])
        for m in range(n_mc)
    )

    for m, (times, ckpt_samples) in enumerate(results):
...
def _run_one_tau_leap_particle(N, w, mu, beta, T, tau, checkpoint_times, seed,
                               corrector, corrector_method, corrector_start,
                               corrector_hyperparameters):
    """
    Worker: run one tau-leap reverse trajectory (with optional corrector).
    Called in parallel by add_tau_leap_reverse.

    Note: unmasking_prob_fn is recreated inside each worker because closures
    containing lambdas cannot be pickled across processes.
    """
    rng = np.random.default_rng(seed)
    unmasking_prob_fn = make_conditional_prob_fn_mixture(w, mu, MASK=-1)

    # Correctors that need unmasking_prob_fn get it injected here, inside the worker
    if corrector and corrector_method in ('PRISM', 'informed_corrector', 'DPC'):
        # Make a local copy so we don't mutate the dict that was passed in
        corrector_hyperparameters = dict(corrector_hyperparameters)
        corrector_hyperparameters['unmasking_prob_fn'] = unmasking_prob_fn

    _, times, ckpt_samples, nfe_count = tau_leap_reverse_masked(
        N, w, mu, beta, T, tau=tau,
        checkpoint_times=checkpoint_times,
        rng=rng,
        unmasking_prob_fn=unmasking_prob_fn,
    ...
    return times, ckpt_samples, nfe_count
...
def add_tau_leap_reverse(samples, w, mu, beta, T, tau,
    ...
    # Strip the unmasking_prob_fn from hyperparameters before pickling — it's a
    # closure and cannot be sent to worker processes. Each worker recreates it.
    hyperparams_for_workers = None
    if corrector_hyperparameters is not None:
        hyperparams_for_workers = {
            k: v for k, v in corrector_hyperparameters.items()
            if k != 'unmasking_prob_fn'
        }

    print(f"Running tau-leaping (tau={tau}) reverse process (parallel)...")
    if corrector:
        print(f"  With corrector: {corrector_method}, start={corrector_start}")
        print(f"  Corrector hyperparameters: {hyperparams_for_workers}")
    ...
    results = Parallel(n_jobs=n_jobs, backend='loky', verbose=1)(
        delayed(_run_one_tau_leap_particle)(
            N, w, mu, beta, T, tau, checkpoint_times, seeds[m],
            corrector, corrector_method, corrector_start, hyperparams_for_workers
        )
        for m in range(n_mc)
    )

    for m, (times, ckpt_samples, nfe_count) in enumerate(results):
        if m == 0:
